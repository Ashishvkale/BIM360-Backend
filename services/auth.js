...
const { FORGE_CLIENT_ID, FORGE_CLIENT_SECRET, FORGE_CALLBACK_URL, INTERNAL_TOKEN_SCOPES, PUBLIC_TOKEN_SCOPES } = require('../config');
...
function getAuthorizationUrl() {
    console.log("getAuthorizationUrl  => " + internalAuthClient.generateAuthUrl());
...
async function authCallbackMiddleware(req, res, next) {
    ...
    console.log("authCallbackMiddleware => " + publicCredentials.access_token + " <= public token", internalCredentials.access_token + " <= internal token", publicCredentials.refresh_token + " <= refresh token")
    next();
}
...
async function authRefreshMiddleware(req, res, next) {
    ...
    if (expires_at < Date.now()) {
        ...
        req.session.refresh_token = publicCredentials.refresh_token;
        req.session.expires_at = Date.now() + internalCredentials.expires_in * 1000;
        console.log("authRefreshMiddleware =>  " + publicCredentials.access_token, internalCredentials.access_token)
    }
